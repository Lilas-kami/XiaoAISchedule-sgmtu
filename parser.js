function scheduleHtmlParser(html) {
        ...
        for (let i = 0; i < ranges.length; i++) {
            if(ranges[i].indexOf("-")!= -1){
                ...
                for (let j = start; j <= end; j++) {
                    ...
                    returnArrays.push(parseInt(ranges[i],10))
                }
        }
        return returnArrays;
    }
    ...
    function uniqueCourses(courses) {
        var seen = {}; // 用来记录已经出现过的序列化课程
        var result = []; // 存放去重后的课程
    
        for (var i = 0; i < courses.length; i++) {
            var course = courses[i];
            var key = JSON.stringify(course); // 将对象序列化为字符串
    
            if (!seen[key]) { // 如果这个字符串没有被记录过
                seen[key] = true; // 记录这个字符串
                result.push(course); // 将这个对象加入结果数组
            }
        }
    
        return result; // 返回去重后的课程数组
    }

    //console.log(result);
    var result = uniqueCourses(tempresult);
    return result
}
